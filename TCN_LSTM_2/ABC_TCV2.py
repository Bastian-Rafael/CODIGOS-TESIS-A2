...
#       3.1) Define aquí tu función de pérdida exactamente igual
def weighted_mse(y_true, y_pred):
    w = K.constant([1.0, 1.0, 5.0, 1.0])
    return K.mean(w * K.square(y_true - y_pred), axis=-1)

#       3.2) Carga el modelo pasándole custom_objects
model = load_model(
    'best_model.keras',
...
S_x_test = model.predict(X, batch_size=32, verbose=1)  
